...
z_where_dim = z_where_scale_dim + z_where_shift_dim
z_pres_dim = 1
z_depth_dim = 1
z_coordinate_dim = 2
z_all_dim = z_what_dim + z_where_dim + z_pres_dim + z_depth_dim + z_coordinate_dim

def unwrap_z(z_all):
    z_what, z_where, z_depth, z_pres, z_coordinate = z_all[:,:,:z_what_dim], \
                                z_all[:,:,z_what_dim:z_what_dim+z_where_dim], \
                                z_all[:,:,z_what_dim+z_where_dim:z_what_dim+z_where_dim+z_depth_dim], \
                                z_all[:,:,z_what_dim+z_where_dim+z_depth_dim:z_what_dim+z_where_dim+z_depth_dim+z_pres_dim], \
                                z_all[:,:,z_what_dim+z_where_dim+z_depth_dim+z_pres_dim:]
    return z_what, z_where, z_depth, z_pres, z_coordinate
