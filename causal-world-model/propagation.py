...
class PropagationCell(nn.Module):
    ...
    def __init__(self, args, z_what_net, glimpse_dec_net):
        ...
        # z where nets
        z_where_transit_bias_net_input_dim = z_coordinate_dim + z_what_dim + z_where_scale_dim + \
                                             z_where_shift_dim + 1 #node type
        ...
        # z what net 
        z_what_transit_net_input_dim = nf + z_where_scale_dim + z_where_shift_dim + z_what_dim
        self.z_what_from_transit_net = nn.Sequential(
            nn.Linear(z_what_transit_net_input_dim, nf),
            nn.CELU(),
            nn.BatchNorm1d(nf),
        ...
        object_transit_inp_dim = z_coordinate_dim + action_dim + 1 #node type

        self.object_transit_net = PropNet(
            node_dim_in=object_transit_inp_dim,
            edge_dim_in=0,
            nf_hidden=nf * 3,
            node_dim_out=z_coordinate_dim,
            edge_dim_out=nf,
            edge_type_num=2,
    ...
    def forward(self, x, act, 
        ...
        '''
        z_what_pre, z_where_pre, z_depth_pre, z_pres_pre, z_coordinate_pre = unwrap_z(z_all_pre)
        bs = z_what_pre.size(0)
        max_num_obj = z_what_pre.size(1)
        device = z_what_pre.device
        obj_mask = (z_pres_pre.view(bs, max_num_obj) != 0).float()
        lengths = obj_mask.sum(dim=-1)
        bns = bs * max_num_obj

        # node_rep: B x N x infer_graph_struct_node_dims
        obj_rep = torch.cat(
            [z_where_pre, z_what_pre, z_depth_pre],
            dim=2
        )
        ...
        if self.node_type == None:
            ...
            self.node_type = gumbel_softmax(node_type_logits.view(bs, max_num_obj)-(1-obj_mask)* 10000000, hard=hard_gumble_softmax)
        ...
        obj_act_inp = torch.cat(
            [z_coordinate_pre, expanded_action],
            dim=2
        )
        #import pdb; pdb.set_trace()
        z_coordinate = self.object_transit_net(
            obj_act_inp,
            None,
            edge_type,
        ...
        z_where_transit_bias_net_inp = torch.cat(
            [z_coordinate, z_what_pre, z_where_pre, node_type.unsqueeze(2)], dim=2
        )
        # bns x dim
        ...
        z_what_pre = z_what_pre.view(bns, -1).contiguous()
        ...
        # transit
        z_what_transit_inp = torch.cat(
            [object_transit_out, z_where, z_what_pre],
            dim=1
        )
        z_what_from_transit_mean, z_what_from_transit_std = \
            self.z_what_from_transit_net(z_what_transit_inp).chunk(2, -1)
        ...
        if self.args.phase_generate:
            ...
            '''
            z_what_mean = z_what_from_enc_mean
            z_what_std = z_what_from_enc_std
            z_what_dist = z_what_encode_dist

        z_what = z_what_dist.rsample()
       
        # z depth transit'
        z_depth_transit_net_inp = torch.cat(
            [object_transit_out, z_what_pre, z_depth_pre],
            dim=1
        )
        ...
        z_pres = z_pres_pre # make it all 1 for now
        ...
        kl_z_what = \
            (kl_divergence(z_what_dist, z_what_transit_dist).sum(1) * \
        ...
        z_where_dummy = z_what.new_ones(bs, max_num_obj, (z_where_scale_dim + z_where_shift_dim)) * .5
        ...
        if self.args.log_phase:
            self.log = {
                'z_what': z_what_all,
                'z_where': z_where_all,
                'z_pres': obj_mask.view(bs, max_num_obj, -1),
                ...
                'z_pres_y': obj_mask.view(bs, max_num_obj, -1),
            ...
            self.log = {}
        z_all = torch.cat([z_what_all, z_where_all,z_depth_all, z_pres], dim=-1)
        return z_all, ids_pre, kl_z_what, kl_z_where, kl_z_depth, \
               kl_z_pres, kl_edge_type, self.log
