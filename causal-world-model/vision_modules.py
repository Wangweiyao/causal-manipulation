...
class HLoss(nn.Module):
    ...
    def forward(self, x, prior=None):
        if prior is None:
            ...
            b = F.softmax(x, dim=1) * (F.log_softmax(x, dim=1) - torch.log(prior).view(-1, x.size(1)))
            #b = -b.sum(1)
            b = b.mean()
        return b
...
class BgEncoder(nn.Module):
    ...
    def forward(self, x):
        bs = x.size(0)
        bg_what_mean, bg_what_std = self.enc(x).reshape(bs, -1).chunk(2, -1)
...
class ZWhatEnc(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc_what = nn.Linear(nf, z_what_dim * 2)
...
class GlimpseDec(nn.Module):
    ...
    def forward(self, x):
        x = self.dec(x.reshape(x.size(0), -1, 1, 1))
...
class ConvLSTMEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        bs = x.size(0)
        img_conv_enc = self.image_enc(x.reshape(-1, x.size(2), x.size(3), x.size(4)))

        img_conv_enc = img_conv_enc.reshape(bs, -1, img_conv_enc.size(-3), img_conv_enc.size(-2), img_conv_enc.size(-1))
