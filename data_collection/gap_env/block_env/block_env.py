...
class SawyerBlockEnv(SawyerXYZEnv):
    def __init__(
        ...
        self.observation_space = Box(0, 1.0, (128,128,6))
        self.view_nbr = None
    ...
    def _set_view_nbr(self, view_nbr):
        self.view_nbr = view_nbr

    def _get_obs(self):
        im = self.sim.render(128, 128, camera_name="agentview")
        im = im.transpose([2,0,1])
        im2s = np.zeros((self.view_nbr,3,128,128))

        chose_cam_ids = np.random.choice(range(15),self.view_nbr,replace=False)
        chose_cam_ids.sort()
        for i, cam_id in enumerate(chose_cam_ids):
          im2 = self.sim.render(128, 128, camera_name=f"movingview{cam_id}")
          im2 = im2.transpose([2,0,1])
          im2s[i] = im2

        im2s = np.array(im2s)

        # print(self.data.get_camera_xmat('agentview'))
        

        # from scipy.spatial.transform import Rotation as R
        # rot = self.data.get_camera_xmat(f'agentview')
        # pos = self.data.get_camera_xpos(f'agentview')
        # r = R.from_matrix(rot)
        # quaternion = r.as_quat()
        # msf=f'[{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}, {quaternion[0]:.3f}, {quaternion[1]:.3f}, {quaternion[2]:.3f}, {quaternion[3]:.3f}],'
        # print(r.as_matrix())
        # print(msf)
        # print(self.data.get_camera_xmat(f'movingview0'))
        # for i in range(15):
        #   rot = self.data.get_camera_xmat(f'movingview{i}')
        #   pos = self.data.get_camera_xpos(f'movingview{i}')
        #   r = R.from_matrix(rot)
        #   quaternion = r.as_quat()
        #   msf=f'[{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}, {quaternion[0]:.3f}, {quaternion[1]:.3f}, {quaternion[2]:.3f}, {quaternion[3]:.3f}],'
        #   print(msf)
          #print(self.data.get_camera_xmat(f'movingview{i}'))
        # print(self.sim.model.cam_fovy[cam_id])
        # fov = self.sim.model.cam_fovy[cam_id]
        # print(1 / np.tan(np.deg2rad(fov) / 2))
        # import pdb; pdb.set_trace()
        obs = np.concatenate([im, self.goalim], 0)
        obs2 = im2s #np.concatenate([im2, self.goalim], 2)
        return obs, obs2, chose_cam_ids
    ...
    def render(self, mode=""):
        return self.sim.render(128, 128, camera_name="agentview")
    ...
    def sample_goal(self):
        ...
        if not self.low_dim:
            self.goalim = self.sim.render(128, 128, camera_name="agentview").transpose([2,0,1])
